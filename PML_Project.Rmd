...
---
<p style = "color: steelblue; font-size: 14pt"> Summary </p>

The data for this project are derived from data collected as part of the Groupware@LES Human Activity Recognition (HAR) research (http://groupware.les.inf.puc-rio.br/har). A 2013 study collected data on six subjects who were novice weightlifters.  All subjects were males between 20-28 years of age.  Correct weightlifting technique for unilateral dumbbell biceps curl was illustrated and feedback was provided to attempt to improve the quality of each subject's weightlifting execution.  Wearable sensors (belt, glove, arm-band, and magnetic dumbbell) were used to record differences between correctly performed bicep curls and the same activity performed incorrectly.  

Five classes were created to classify subject performance: Class A denoted correct technique.  Classes B, C, D, and E indicated four different incorrect variations (throwing elbows forward, lifting the dumbbell only part way, lowering the dumbbell only partway, and throwing the hips forward, respectively).

In this project, a model was trained to predict the subject performance class using the data collected. 

<p style = "color: steelblue; font-size: 12pt"> Cleaning and organizing the data: </p>
The dataset provided contained 19622 observations of 160 variables.  Because the randomForest package in R requires no more than 53 predictor variables, that became the targeted number of columns.  Columns were first eliminated from the data if more than 10 percent (1962 rows) of the data was missing, thereby reducing the number of variables in the dataset to 60. The data was again subset to eliminate the first 7 rows, as these variables were not potentially predictive of class beyond these subjects.
...
The machine learning algorithm selected was <b>random forest</b> becuase a large number of variables were provided, and random forest tends to have a high degree of accuracy in the presence of increased noise. 
...
```
<p style = "color: steelblue; font-size: 12pt"> Concluding notes: </p>
A beginner to weight training who chooses to get feedback from wearables on the correctness of his technique would do well to prioritize the use of a belt and magnet dumbbell.  These two wearables provide six of the eight top predictors of the correctness-of-technique classification: correct technique, throwing elbows forward, lifting the dumbbell only part way, lowering the dumbbell only partway, and throwing the hips forward.